...
export const buyMuon = async (muon, fromCurrency, toCurrency, amountIn, amountOut, fromSymbol, amount, time, account, chainId, validChainId = 1, web3, callback) => {
    ...
    try {
        ...
        if (output.error) {
            const errorMessage = output.error.message ? output.error.message : output.error ? output.error : ""
