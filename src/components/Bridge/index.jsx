...
const Bridge = () => {
                ...
                for (let index = 0; index < dest.length; index++) {
                    ...
                    try {
                        ...
                        for (let index = 0; index < pendingIndex.length; index++) {
                            ...
                            let txObj = await Contract[chain.network].methods.getTx(account).call()
                            let currentBlockNo = txObj[Object.keys(txObj)[15]]

                            claim.remainingBlock = Number(claim.txBlockNo) + blockTimes[chain.network] - Number(currentBlockNo)
                            claims.push(claim)
                        }
                    } catch (error) {
    ...
    const handleDeposit = () => {
        try {
            ...
            sendTransactionDeposit(
                ...
                `deposit`,
                [amountWie, bridge.to.chainId, bridge.from.tokenId, "0"],
                account,
                chainId,
    ...
    const handleClaim = async (claim, network) => {
        try {
            ...
            let networkName = chains.find(
                (item) => item.network === Number(claim.fromChain)
            ).networkName
            console.log({
                address: BridgeContractAddress[Number(claim.fromChain)],
                method: 'getTx',
                params: [claim.txId],
                abi,
                network: networkName
    ...
    return (
        ...
        <img src="/img/bridge/image 1.svg" alt="logo" className="ftm-logo" /> */}
                <div className="wrap-bridge-box">
                    ...
                    {/* <p style={{ textAlign: "center", marginTop: "10px", display: "flex", alignItems: "center", justifyContent: "center", color: "#0000009b" }}>Secured by <img style={{ width: "30px", marginLeft: "8px", display: "none" }} src="/tokens/muon.svg" alt="" />Muon Network</p> */}
                </div>
