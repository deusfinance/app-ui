  ...
  const pc =
    poolBalance !== null && ceiling !== null
      ? formatUnitAmount(
        ...
        " / " +
        formatUnitAmount(new BigNumber(ceiling).div(TEN.pow(decimals - 6)))
      : null;
  const av = pc
    ? formatUnitAmount(
        new BigNumber(ceiling).minus(poolBalance).div(TEN.pow(decimals - 6))
      )
    : null;
...
export const makeCostDataRedeem = (
  collatRatio,
  poolBalance,
  chainId = ChainId.ETH,
  depositAmount = 0,
...
export const makeCostDataBuyBack = (
  ...
  const cToken = collateralToken[chainId];
  const deusPrice =
    deus_price !== null && !isNaN(deus_price)
      ? `$${new BigNumber(deus_price).toFixed(3)}`
      : null;
  const deiPrice =
    dei_price !== null && !isNaN(dei_price)
      ? `$${new BigNumber(dei_price).toFixed(3)}`
      : null;
  const p = pool ? pool : null;
  const bb =
    buyBack !== null && !isNaN(buyBack)
      ? `${formatUnitAmount(buyBack)} DEUS`
      : null;
  const rc =
    recollateralize !== null && !isNaN(recollateralize)
      ? `${formatUnitAmount(recollateralize)} ${cToken?.symbol}`
      : null;
...
export const getSspData = (chainId = ChainId.FTM, oracleResponse) => {
  ...
  const leftMintableParams =
    chainId !== ChainId.FTM
      ? { params: [getToWei(oracleResponse.deus_price, 6).toFixed(0)] }
      : {};
...
export const DeiDeposit = (depositedToken, amount, address, web3) => {
  ...
  return getDeiStakingContract(web3, address).methods.deposit(
    getToWei(amount, depositedToken.decimals).toFixed(0)
  );
};

export const DeiWithdraw = (withdrawToken, amount, address, web3) => {
  return getDeiStakingContract(web3, address).methods.withdraw(
    getToWei(amount, withdrawToken.decimals).toFixed(0)
  );
};

export const buyBackDEUS = (
  amountIn,
  deus_price,
  expire_block,
  signature,
  pool_collateral_price = "0",
  account,
  chainId,
  web3
) => {
  return getCollateralPoolContract(web3, chainId).methods.buyBackDEUS(
    amountIn,
    [pool_collateral_price],
    deus_price,
    expire_block,
    [signature]
  );
...
export const RecollateralizeDEI = (
  collateral_price,
  deus_price,
  expire_block,
  signature,
  ...
  return getCollateralPoolContract(web3, chainId).methods.recollateralizeDEI([
    amountIn,
    pool_collateral_price,
    [collateral_price],
    deus_price,
...
export const mintDeiSSP = (amountIn, chainId, web3) => {
  return getSSPContract(web3, chainId).methods.buyDei(amountIn);
};
export const mintDeiSSPv4 = (amountIn, chainId, web3) => {
  return getSSPV4Contract(web3, chainId).methods.buyDei(amountIn);
};

export const mintDeiSSPWithOracle = (amountIn, result, chainId, web3) => {
  console.log(amountIn, result.deus_price, result.expire_block, [
    result.signature,
  ]);
  return getSSPContract(web3, chainId).methods.buyDei(
    amountIn,
    result.deus_price,
    result.expire_block,
    [result.signature]
...
export const mint1t1DEI = (collateral_amount, chainId, web3) => {
  return getCollateralPoolContract(web3, chainId).methods.mint1t1DEI(
    collateral_amount
  );
};

export const mintFractional = (
  collateral_amount,
  deus_amount,
  deus_current_price,
  expireBlock,
  ...
  return getCollateralPoolContract(web3, chainId).methods.mintFractionalDEI(
    collateral_amount,
    deus_amount,
    deus_current_price,
    expireBlock,
...
export const mintAlgorithmic = (
  ...
  return getCollateralPoolContract(web3, chainId).methods.mintAlgorithmicDEI(
    deus_amount_d18,
    deus_current_price,
    expire_block,
    [signature]
...
export const redeem1to1Dei = (amountIn, chainId, web3) => {
  return getCollateralPoolContract(web3, chainId).methods.redeem1t1DEI(
    amountIn
  );
};

export const redeemFractionalDei = (amountIn, chainId, web3) => {
  return getCollateralPoolContract(web3, chainId).methods.redeemFractionalDEI(
    amountIn
  );
};

export const redeemAlgorithmicDei = (amountIn, chainId, web3) => {
  return getCollateralPoolContract(web3, chainId).methods.redeemAlgorithmicDEI(
    amountIn
  );
};

export const collectCollateral = (web3, chainId = ChainId.ETH) => {
  return getCollateralPoolContract(web3, chainId).methods.collectCollateral();
};

export const collectDeus = (
  web3,
  chainId = ChainId.ETH,
  price,
  id,
  ...
  return getCollateralPoolContract(web3, chainId).methods.collectDeus(
    price,
    id,
    [signatures]
  );
...
//READ FUNCTIONS
export const getUsdcTwapOracle = async (
  web3,
  tokenIn,
  amountIn,
  timestamp,
  ...
  return getUsdcTwapOracleContract(web3, ChainId.FTM)
    .methods.twap(tokenIn, amountIn, timestamp, duration)
    .call();
};

export const getDeiInfo = async (
  web3,
  chainId = ChainId.ETH,
  collat_usd_price = collatUsdPrice
) => {
  ...
  return getDeiContract(web3, chainId).methods.dei_info(collaterals).call();
};
...
export const isProxyMinter = (token, isPair, collatRatio, chainId) => {
  ...
  if (
    (collatRatio === 100 &&
      token.symbol === collateralToken[chainId]?.symbol &&
      !isPair) ||
    (collatRatio === 0 && token.symbol === "DEUS" && !isPair) ||
    (collatRatio > 0 && collatRatio < 100 && isPair)
  )
    return false;
  return true;
};

export const isSspMinter = (
  token,
  isPair,
  amountIn,
  lowerBound,
  ...
  if (
    token.address !== SSP_COLLATERAL_ADDRESS[chainId] ||
    !SSP_ADDRESS[chainId]
  ) {
    return false;
  }
...
export const checkSSPvalidInput = (
  amountIn,
  lowerBound,
  topBound,
  deiLeftInSSP
  ...
  if (
    (!amountIn && !new BigNumber(lowerBound).isZero()) ||
    new BigNumber(amountIn).comparedTo(lowerBound) < 0 ||
    new BigNumber(amountIn).comparedTo(topBound) > 0
  ) {
  ...
  if (
    new BigNumber(amountIn).comparedTo(deiLeftInSSP) > 0 ||
    new BigNumber(deiLeftInSSP).comparedTo(1_000) < 0
  ) {
...
export const getAmountOutDeusSwap = async (
  fromCurrency,
  amountIn,
  deus_price,
  collateral_price,
  ...
  return getDeusSwapContract(web3, chainId)
    .methods[method](...params)
    .call();
};

export const getAmountOutProxy = async (
  fromCurrency,
  amountIn,
  deus_price,
  collateral_price,
  ...
  // console.log(method, params);
  return getNewProxyMinterContract(web3, chainId)
    .methods[method](...params)
    .call();
};

export const getZapAmountsOut = async (
  currency,
  amountInToWei,
  zapperAddress,
  result,
  ...
  const collateral_price_toWei = getToWei(result.collateral_price, 6).toFixed(
    0
  );
  ...
  if (zapperAddress === DEI_COLLATERAL_ZAP[chainId]) {
    if (chainId === ChainId.FTM) {
      return await getZapContract(web3, zapperAddress, chainId)
        .methods.getAmountOut(
          [
            amountInToWei,
            deus_price_toWei,
            collateral_price_toWei,
            [...erc20Path],
            [...toNativePath],
          ],
          useMinter
    ...
    if (currency.symbol === "DEUS" && chainId === ChainId.MATIC) {
      ...
      return await getZapContract(web3, zapperAddress, chainId)
        .methods.getAmountOut(amountInToWei, [...toNativePath])
        .call();
    }
    ...
    return getZapContract(web3, zapperAddress, chainId)
      .methods.getAmountOutLPDEUS(amountInToWei)
      .call();
  } else if (currency.address === DEI_ADDRESS[chainId]) {
    return getZapContract(web3, zapperAddress, chainId)
      .methods.getAmountOutLPDEI(amountInToWei)
      .call();
  }
  return getZapContract(web3, zapperAddress, chainId)
    .methods.getAmountOutLPERC20ORNativecoin(
      amountInToWei,
      deus_price_toWei,
      collateral_price_toWei,
      erc20Path
...
export const zapIn = (
  ...
  if (zapperAddress === DEI_DEUS_ZAP[chainId]) {
    if (currency.address === "0x") {
      return getZapContract(
        web3,
        zapperAddress,
        chainId
      ).methods.zapInNativecoin(
        proxyTuple,
        minLpAmount,
        erc20Path,
        transferResidual
      ...
      return getZapContract(web3, zapperAddress, chainId).methods.zapInDEUS(
        amountIn,
        minLpAmount,
        transferResidual
      );
    } else if (currency.address === DEI_ADDRESS[chainId]) {
      return getZapContract(web3, zapperAddress, chainId).methods.zapInDEI(
        amountIn,
        minLpAmount,
        transferResidual
      );
    }
    return getZapContract(web3, zapperAddress, chainId).methods.zapInERC20(
      proxyTuple,
      amountIn,
      minLpAmount,
      erc20Path,
      ...
      if (chainId === ChainId.FTM) {
        if (amountOutParams[5] === true) {
          return getZapContract(
            ...
            zapperAddress,
            chainId
          ).methods.zapInNativecoinWithMinting(
            minLpAmount,
            transferResidual,
            proxyTuple,
            erc20Path,
        ...
        return getZapContract(
          ...
          zapperAddress,
          chainId
        ).methods.zapInNativecoinWithoutMinting(
          minLpAmount,
          transferResidual,
          erc20Path,
          amountOutParams[4]
    ...
    if (chainId === ChainId.FTM) {
      ...
      return getZapContract(
        ...
        zapperAddress,
        chainId
      ).methods.zapInERC20WithoutMinting(
        amountIn,
        minLpAmount,
        transferResidual,
        erc20Path,
